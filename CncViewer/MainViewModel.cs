...
using ConnectionData = CncViewer.Models.Connection.ConnectionData;
using CncLinksModel = CncViewer.Models.Connection.Links;
using CncLinksVM = CncViewer.Connection.ViewModels.Links;
...
namespace CncViewer
{
    public class MainViewModel : MachineElements.ViewModels.MainViewModel, IStepExecutionInfoProvider
    {
        ...
        private ICommand _openConnectionDataCommand;
        public ICommand OpenConnectionDataCommand { get { return _openConnectionDataCommand ?? (_openConnectionDataCommand = new RelayCommand(() => OpenConnectionDataCommandImplementation(), () => true )); } }

        private ICommand _clearConnectionDataCommand;
        public ICommand ClearConnectionDataCommand { get { return _clearConnectionDataCommand ?? (_clearConnectionDataCommand = new RelayCommand(() => ClearConnectionDataCommandImplementation(), () => true )); } }

        private ICommand _startConnectionCommand;
        public ICommand StartConnectionCommand { get { return _startConnectionCommand ?? (_startConnectionCommand = new RelayCommand(() => StartConnectionCommandImplementation(), () => true )); } }

        private ICommand _stopConnectionCommand;
        public ICommand StopConnectionCommand { get { return _stopConnectionCommand ?? (_stopConnectionCommand = new RelayCommand(() => StopConnectionCommandImplementation(), () => true )); } }
        ...
        private void OpenConnectionDataCommandImplementation()
        {
        ...
        private CncLinksVM.LinkViewModel ToViewModel(CncLinksModel.LinkData model)
        {
            if (model is CncLinksModel.LinearLinkData lld)
            {
                return new CncLinksVM.LinearLinkViewModel(lld.LinkId, lld.Variable) { Description = model.Descrition };
            }
            else if (model is CncLinksModel.TwoPosLinkData tpld)
            {
                return new CncLinksVM.TwoPosLinkViewModel(tpld.LinkId, tpld.Variable) { Description = model.Descrition };
            }
            else
            {
        ...
        private void ClearConnectionDataCommandImplementation()
        {
            throw new NotImplementedException();
        }

        private void StartConnectionCommandImplementation() => MessengerInstance.Send(new StartReadingMessage());

        private void StopConnectionCommandImplementation() => MessengerInstance.Send(new StopReadingMessage());
